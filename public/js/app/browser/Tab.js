...
export class Tab {
    ...
    constructor(browserApp) {
        ...
        this.topbarTab = $(`<li class="site">
            <img src="/image/icon/${icon}" alt="site icon" class="siteicon">
            <span>${title}</span>
            <div class="siteclose">
                <img src="/image/icon/close.webp" alt="close icon" class="sitecloseicon">
            </div>
        </li>`);
    ...
    async goToWebsite(historyIndex) {
        ...
        let site = await $.get("html/website/" + page + ".html");
        this.topbarTab.find("span").text(this.website.title);
        this.topbarTab.find(".siteicon").attr("src", "/image/icon/" + this.website.icon);
        this.browserApp.appManager.addStyle("css/website/" + page + ".css");
        if (this.window) {
            this.window.remove();
        }
